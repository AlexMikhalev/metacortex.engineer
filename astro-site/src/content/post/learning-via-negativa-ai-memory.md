...
subtitle: "Every failed command contains information. The trick is capturing that signal instead of ignoring the noise."
slug: "learning-via-negativa-ai-memory"
description: "How Terraphim's Learning via Negativa system captures failed commands, builds a correction knowledge graph, and prevents AI agents from repeating the same mistakes."
tags: ["rust", "ai-agents", "knowledge-graph", "developer-tools"]
...
This differs from the [self-correction approach I wrote about earlier](/post/teaching-ai-agents-self-correction/) in an important way. That post covers the FINAL Bench and MetaCog scaffold -- the mechanical system for catching and retrying failures. Learning via Negativa covers the *philosophy*: building a persistent memory from those failures so the same mistake never happens twice.
...
*Terraphim AI is a privacy-first, deterministic AI assistant built with Rust. [Learn more](https://terraphim.ai).*
